...
function deObjetoAmatriz(objeto){
  ...
  var newObjectArray = [];
  for(x in objeto) {
    newObjectArray.push([x, objeto[x]]);
  }
...
function capToFront(s) {
  ...
  return cadena.concat('', cadenaDos);
}
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  var phrase = str.split(' ').map(function (x) {
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
  const newString = cadena.toLowerCase().split('').filter(x => x !== 'a' && x !== 'b' && x !== 'c').join('');
  return newString;
}
